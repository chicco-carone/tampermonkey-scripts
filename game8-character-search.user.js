...
(function() {
    ...
    function getAllCharacters() {
        const characters = [];
        const cards = document.querySelectorAll('.js-discription-tooltip.c-tooltip__btn');
    ...
    function showResults(matches) {
        ...
        if (matches.length === 0) {
            resultDiv.innerHTML = '<div style="color: #888; font-size: 12px;">No characters found</div>';
            return;
        }
        ...
        matches.forEach(match => {
            ...
            item.addEventListener('click', () => {
                const allCards = document.querySelectorAll('.js-discription-tooltip.c-tooltip__btn');
                const card = Array.from(allCards).find(c => {
                    const img = c.querySelector('img');
                    return img && img.alt && new RegExp(escapeRegex(match.name), 'i').test(img.alt);
                });
