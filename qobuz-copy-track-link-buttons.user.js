...
function clicky(i) {
  ...
  $("button.track__item--button")
    .eq(i - 1)
    .html(
      "Copied! &nbsp <img src='https://slavart.gamesdrive.net/favicon.ico' width='24px'/>",
    );
  setTimeout(function () {
...
function main() {
  ...
  $(".track__items")
    .find("div:last")
    .after(
      '<button class="track__item--button" style="padding: 0px; margin-left; cursor: pointer;" type="button"> copy track link </button>',
    );
  $("button.track__item.popin-addtocart__cta").each(function (i) {
    $(this)
      .parent()
      .find("button.track__item--button")
      .attr("onclick", `clicky(${i + 1})`);
    $(this).remove();
  });
}
